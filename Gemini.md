...
1. 内联组件级别的媒体视频与封面渲染
2. 依据各平台不同封控能力构建的**全动态属性探针约束矩阵**。针对 B站，实现了「授权执行模式 (免登/受限)」x「爬取深度策略 (简易/详细)」的笛卡尔二维矩阵，底层 API 在 `search_by_type()` 与 `get_info()` 之间的调用权限及对投币/分享的截断防御策略已完全映射于前台 14 行原子参数表，AI 在自动化部署前可直观明眸所有底层执行约束！
